...
export function spawnHostProcess(
  ...
  const child = electronUtilityProcess.fork(hostModulePath, [], {
    ...
    env: {
      ...
      // code-signing identity is a nested Electron helper (NOT dev.dontthink.app). Publish THIS (main
      // Electron) process's pid — which IS dev.dontthink.app — so helperLauncher passes it as
