...
export function scanZCodeDataDirectoryInWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    const abort = () => {
      ...
      finish(() => reject(new DOMException("Don’t-think data size scan aborted", "AbortError")));
    };
        ...
        reject(
          new Error(
            response.ok === false && typeof response.error === "string"
              ? response.error
              : "Invalid Don’t-think data size worker response",
          ),
        ),
      );
    });
    ...
    worker.once("exit", (code) => {
      if (code !== 0) {
        finish(() => reject(new Error(`Don’t-think data size worker exited with code ${code}`)));
      }
    });
