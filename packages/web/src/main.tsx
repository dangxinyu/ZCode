...
function renderWebAuthCallbackPage(): void {
  document.title = "Don’t-think - Sign In";
...
async function renderConversationSharePage(): Promise<void> {
  ...
  document.title = routeLocale === "zh-CN" ? "Don’t-think 会话分享" : "Don’t-think Conversation Share";
...
function renderWebBootstrapError(error: unknown): void {
  document.title = "Don’t-think - Web";
  root.render(
...
async function bootstrapWebApp() {
  ...
  try {
    ...
    document.title = "Don’t-think - Web + Server";
